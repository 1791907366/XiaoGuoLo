...
// ================= 全局表单 AJAX 提交 =================
// 所有后台表单改为 AJAX 提交，成功/失败用 layer 弹窗提示，不再跳转提示页
// 后端 success()/error()/alert_back() 已内置 is_ajax() 判断，AJAX 请求自动返回 JSON {code,data,tourl}
$(function () {
    $(document).on('submit', 'form.layui-form', function (e) {
        var $form = $(this);
        // 排除登录表单（已有独立 AJAX 逻辑）
        if ($form.attr('id') === 'dologin') return;
        // GET 搜索表单保持原样跳转
        if (($form.attr('method') || 'get').toLowerCase() === 'get') return;
        // 已绑定过则跳过（防止重复触发）
        ...
        var $btn = $form.find('button[lay-submit]');
        var btnTxt = $btn.length ? $btn.html() : '';
        if ($btn.length) {
            $btn.prop('disabled', true).html('<i class="fa fa-circle-o-notch fa-spin"></i> 提交中...');
        }
        $.ajax({
            type: 'POST',
            url: url,
            ...
            success: function (res) {
                if ($btn.length) $btn.prop('disabled', false).html(btnTxt);
                if (res && res.code == 1) {
                    var msg = res.data || '操作成功！';
                    var tourl = res.tourl || '';
                    ...
                    layer.msg((res && res.data) || '操作失败！', {icon: 2, time: 2000});
                }
            },
            error: function (xhr) {
                if ($btn.length) $btn.prop('disabled', false).html(btnTxt);
                // 后端 error() 返回 404 + JSON body，尝试解析提示
                var msg = '操作失败，请重试！';
                try {
